...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (toggleFiltersBtn && filterForm && toggleText) {
    ...
    function fetchFilteredData() {
        ...
        const headFilter = document.getElementById('head-filter')?.value || 'all';
        const vendorFilter = document.getElementById('vendor-filter')?.value || 'all';
        const statusFilter = document.getElementById('status-filter')?.value || 'all';
        
        // Show loading state with enhanced animation on all chart containers
        document.querySelectorAll('.chart-container').forEach((container, index) => {
            // Create a modern loading spinner if not already present
                ...
                <tr>
                    <td colspan="6" class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex justify-center items-center">
                            <div class="loading-spinner mr-3" style="width: 24px; height: 24px;"></div>
                            <span class="text-gray-500">Loading expense data...</span>
                        </div>
                    </td>
                </tr>
            `;
        ...
        // Make AJAX request to get filtered data with timeout handling
        const fetchTimeout = setTimeout(() => {
            if (dashboardLoading && !dashboardLoading.classList.contains('hidden')) {
                showNotification('Request is taking longer than expected. Please wait...', 'info');
            }
        }, 5000);
        ...
        .catch(error => {
            ...
            showNotification(`Error loading data: ${error.message || 'Unknown error'}. Please try again.`, 'error');
        });
    }
    ...
    function exportDashboardData() {
        ...
        // Create a clone of the dashboard content for export
        const dashboardContent = document.querySelector('.expense-content');
        if (!dashboardContent) {
            showNotification('Unable to export dashboard content', 'error');
            return;
        }
        ...
        try {
            ...
            title.textContent = 'MIS Analytics Dashboard Export';
            title.style.textAlign = 'center';
            title.style.marginBottom = '20px';
            title.style.color = '#004d66';
            ...
            // Append elements to export container
            ...
            const elementsToHide = contentClone.querySelectorAll('.filter-container, #toggle-filters');
            ...
            if (typeof html2pdf === 'undefined') {
                // If html2pdf is not loaded, use window.print() as fallback
                const printWindow = window.open('', '_blank');
                printWindow.document.write('<html><head><title>MIS Dashboard Export</title>');
                ...
                // Use html2pdf for better PDF generation
                const opt = {
                    margin: [10, 10],
                    filename: `MIS_Dashboard_Export_${now.toISOString().split('T')[0]}.pdf`,
                    image: { type: 'jpeg', quality: 0.98 },
                    html2canvas: { scale: 2, useCORS: true },
                    jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' }
                };
                
                // Temporarily append to body for html2pdf to work
